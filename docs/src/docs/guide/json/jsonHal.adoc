...
The JSON views plugin for Grace provides HAL support out-of-the-box. All JSON views have access to the `hal` instance which implements link:{api}/grails/plugin/json/view/api/HalViewHelper.html[HalViewHelper].
...
For example given the following configuration in `app/conf/application.yml`:
...
The JSON views plugin for Grace provides navigable pagination support.
