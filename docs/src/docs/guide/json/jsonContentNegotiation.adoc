...
GSON views integrate with Grace' content negotiation infrastructure. For example if you create two views called `app/views/book/show/show.gsp` (for HTML) and `app/views/book/show/show.gson` (for JSON), you can then define the following action:

[source,groovy]
.`app/controllers/myapp/BookController.groovy`
----
def show() {
...
For example given a class called `demo.Book` you can create `app/views/book/_book.gson` and whenever `respond` is called with an instance of `Book` Grace will render `_book.gson`.
...
Then you can create a corresponding `app/views/book/index.gson` file that renders each book:

.app/views/book/index.gson
[source,groovy]
----
...
NOTE: When responding with a list of objects Grace automatically appends the suffix "List" to the model name, so in this case the model name is `bookList`

By calling the `tmpl.book(..)` method with the list of books the `app/views/book/_book.gson` template will be rendered for each one and a JSON array returned.

== Global Default Template

You can also define a `/object/_object` template that is rendered by default if no other template is found during content negotiation. To do this create a file called `/app/views/object/_object.gson` where the name of the model is `object`, for example:
...
Grace will then use the `ACCEPT-VERSION` header when resolving the view.
...
Grace Framework takes into account a number of factors when attempting to resolve the view including the content type, version and locale.
...
Some REST APIs use the notion of custom mime types to represent resources. Within Grace you can for example define custom mime types in `app/conf/application.yml`:
