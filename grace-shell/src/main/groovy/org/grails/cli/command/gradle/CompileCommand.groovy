 ...
 */
@CompileStatic
class CompileCommand implements ProjectCommand {

    static final String USAGE = 'grace compile'
...
'''
    ...
    final CommandDescription description = new CommandDescription(name, 'Compiles a Grace application', USAGE, EXAMPLES)
    ...
    boolean handle(ExecutionContext executionContext) {
        ...
        try {
            GradleUtil.runBuildWithConsoleOutput(executionContext) { BuildLauncher buildLauncher ->
                buildLauncher.withArguments('classes')
            }
            return true
        }
