...
class FormFieldsTagLib {
    ...
    def with = { attrs, body ->
        if (!attrs[BEAN_ATTR]) {
            throwTagError("Tag [with] is missing required attribute [$BEAN_ATTR]")
        }
    ...
    def all = { attrs ->
        if (!attrs[BEAN_ATTR]) {
            throwTagError("Tag [all] is missing required attribute [$BEAN_ATTR]")
        }
        BeanAndPrefix beanPrefix = resolveBeanAndPrefix(attrs[BEAN_ATTR], attrs.prefix, attrs)
        try {
            beanStack.push(beanPrefix)
    ...
    def field = { attrs, body ->
        ...
        if (attrs.containsKey(BEAN_ATTR) && !attrs[BEAN_ATTR]) {
            throwTagError("Tag [field] requires a non-null value for attribute [$BEAN_ATTR]")
        }
        if (!attrs[PROPERTY_ATTR]) {
            throwTagError("Tag [field] is missing required attribute [$PROPERTY_ATTR]")
        }
        ...
        if (propertyAccessor.domainProperty instanceof Embedded) {
            renderEmbeddedProperties(bean, propertyAccessor,
                    attrs + [templates: templatesFolder, field: fieldFolder, input: widgetFolder, theme: theme])
        } else {
    ...
    def widget = { attrs ->
        def bean = resolveBean(attrs.remove(BEAN_ATTR))
        if (!bean) {
            throwTagError("Tag [input] is missing required attribute [$BEAN_ATTR]")
        }
        if (!attrs.property) {
            throwTagError('Tag [input] is missing required attribute [property]')
        }
    ...
    def displayWidget = { attrs ->
        def bean = resolveBean(attrs.remove(BEAN_ATTR))
        if (!bean) {
            throwTagError("Tag [displayWidget] is missing required attribute [$BEAN_ATTR]")
        }
        if (!attrs[PROPERTY_ATTR]) {
            throwTagError('Tag [displayWidget] is missing required attribute [property]')
        }
    ...
    def display = { attrs, body ->
        ...
        def bean = resolveBean(attrs.remove(BEAN_ATTR))
        if (!bean) {
            throwTagError("Tag [display] is missing required attribute [$BEAN_ATTR]")
        }
        ...
        if (property == null) {
            ...
            String displayWrapperTemplateName = formFieldsTemplateService.getTemplateFor('displayWrapper')
            Map template = formFieldsTemplateService.findTemplate(propertyAccessor, displayWrapperTemplateName, wrapperFolderTouse, theme)
            if (template) {
                out << render(template: template.path, plugin: template.plugin, model: model + [attrs: wrapperAttrs] + wrapperAttrs)
            } else {
    ...
    private void renderEmbeddedProperties(bean, BeanPropertyAccessor propertyAccessor, attrs) {
        ...
        out << applyLayout(name: '_fields/embedded', params: [type: toPropertyNameFormat(propertyAccessor.propertyType), legend: legend]) {
            ...
            for (embeddedProp in resolvePersistentProperties(prop.associatedEntity, attrs)) {
                ...
                out << field(bean: bean, property: propertyPath, prefix: attrs.prefix, default: attrs.default,
                        field: attrs.field, widget: attrs.widget, theme: attrs[THEME_ATTR])
            }
        }
    }
    ...
    private CharSequence renderDisplayWidget(BeanPropertyAccessor propertyAccessor, Map model, Map attrs = [:], String widgetFolder, String theme) {
        ...
        if (!template) {
            String displayWidgetTemplateName = formFieldsTemplateService.getTemplateFor('displayWidget')
    ...
    private CharSequence resolveLabelText(BeanPropertyAccessor propertyAccessor, Map attrs) {
        ...
        return labelText ?: propertyAccessor.defaultLabel
    }
    ...
    CharSequence renderDefaultInput(BeanPropertyAccessor propertyAccessor, Map model, Map attrs = [:]) {
        ...
        // TODO: configurable how this gets output? Some people prefer required="required"
        if (model.invalid) {
            attrs.invalid = ''
        }
        if (!isEditable(constrained)) {
            attrs.readonly = ''
        }
        ...
        } else if (model.type in [Date, Calendar, java.sql.Date, java.sql.Time, java.time.Instant,
                                  java.time.LocalTime, java.time.LocalDateTime, java.time.LocalDate,
    ...
    CharSequence renderDateTimeInput(Map model, Map attrs) {
        attrs.precision = (model.type in [java.sql.Time, java.time.Instant, java.time.LocalTime, java.time.LocalDateTime,
                                          java.time.ZonedDateTime, java.time.OffsetDateTime]) ? 'minute' : 'day'
        if (!model.required) {
            attrs.noSelection = ['': '']
            attrs.default = 'none'
        }
    ...
    CharSequence renderStringInput(Map model, Map attrs) {
        ...
        if (constrained?.matches) {
            attrs.pattern = constrained.matches
        }
        if (constrained?.maxSize) {
            attrs.maxlength = constrained.maxSize
        }
    ...
    CharSequence renderNumericInput(BeanPropertyAccessor propertyAccessor, Map model, Map attrs) {
        ...
        if (!attrs.type && constrained?.inList) {
            ...
            if (constrained?.scale != null) {
                attrs.step = "0.${'0' * (constrained.scale - 1)}1"
            }
            if (constrained?.min != null) {
                attrs.min = constrained.min
            }
            if (constrained?.max != null) {
                attrs.max = constrained.max
            }
        }
    ...
    protected Locale getLocale() {
        HttpServletRequest request = GrailsWebRequest.lookup()?.currentRequest
    ...
    protected String getDefaultNumberType(Map model) {
        ...
        String typeName = modelType.simpleName.toLowerCase(Locale.ROOT)
    ...
    private CharSequence renderOneToManyInput(Map model, Map attrs) {
        ...
        buffer << g.link(controller: controllerName, action: 'create',
                params: [("${beanClass.decapitalizedName}.id".toString()): model.bean.id], addLabel)
        buffer.buffer
    }
    ...
    private CharSequence displayEmbedded(bean, PersistentEntity domainClass, Map attrs, String templatesFolder, String theme) {
        Writer buffer = new FastStringWriter()
        List<DomainProperty> properties = domainModelService.getOutputProperties(domainClass)
