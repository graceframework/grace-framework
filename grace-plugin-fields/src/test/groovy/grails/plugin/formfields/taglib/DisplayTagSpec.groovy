...
class DisplayTagSpec extends AbstractFormFieldsTagLibSpec implements TagLibUnitTest<FormFieldsTagLib> {
    ...
    void 'display tag allows to specify the except'() {
        ...
        def result = applyTemplate(
                '<f:display bean="personInstance" except="salutation,grailsDeveloper,picture,anotherPicture,password,dateOfBirth,emails"/>',
    ...
    void 'renders date values using g:formatDate'() {
        expect:
        def output = applyTemplate('<f:display bean="personInstance" property="dateOfBirth"/>', [personInstance: personInstance])
        output ==~ /1987-04-19 00:00:00 [A-Z]{3,4}/
    }
    ...
    void 'displays using template if one is present'() {
        ...
        expect:
        def output = applyTemplate('<f:display bean="personInstance" property="name"/>', [personInstance: personInstance])
        output == '<dt>Name</dt><dd>Bart Simpson</dd>'
    }
    ...
    void 'supports theme for templates'() {
        ...
        expect:
        def output = applyTemplate('<f:display bean="personInstance" property="name" theme="test"/>', [personInstance: personInstance])
        output == '<theme>Name-Bart Simpson</theme>'
    }
