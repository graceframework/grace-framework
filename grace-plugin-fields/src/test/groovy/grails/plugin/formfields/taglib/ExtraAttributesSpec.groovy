...
class ExtraAttributesSpec extends AbstractFormFieldsTagLibSpec implements TagLibUnitTest<FormFieldsTagLib> {
    ...
    void 'arbitrary attributes on f:input are passed to the default input'() {
        expect:
        def output = applyTemplate('<f:input bean="personInstance" property="name" foo="bar"/>', [personInstance: personInstance])
        output == '<input type="text" foo="bar" name="name" value="Bart Simpson" required="" id="name" />'
    }
    ...
    void 'arbitrary attributes prefixed with input- are not passed to the field template'() {
        ...
        expect:
        def output = applyTemplate('<f:field bean="personInstance" property="name" input-foo="bar" />', [personInstance: personInstance])
        output == '<foo></foo>'
    }
    ...
    void 'arbitrary attributes prefixed with input- are not passed to the display template'() {
        ...
        expect:
        def output = applyTemplate('<f:display bean="personInstance" property="name" input-foo="bar" />', [personInstance: personInstance])
        output == '<foo></foo>'
    }
    ...
    void 'arbitrary attributes prefixed with widget- are not passed to the display template (if it is configured as the prefix)'() {
        ...
        expect:
        def output = applyTemplate('<f:display bean="personInstance" property="name" widget-foo="bar" />', [personInstance: personInstance])
        output == '<foo></foo>'
    }
