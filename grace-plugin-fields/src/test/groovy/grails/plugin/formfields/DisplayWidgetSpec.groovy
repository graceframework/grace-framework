...
class DisplayWidgetSpec extends AbstractFormFieldsTagLibSpec implements TagLibUnitTest<FormFieldsTagLib> {
    ...
    void 'f:displayWidget without template and a date value renders the formatted date'() {
        expect:
        applyTemplate('<f:displayWidget bean="personInstance" property="dateOfBirth"/>', [personInstance: personInstance])
                == applyTemplate('<g:formatDate date="${personInstance.dateOfBirth}"/>', [personInstance: personInstance])
    }

    void 'f:displayWidget without template and a boolean value renders the formatted boolean'() {
        expect:
        applyTemplate('<f:displayWidget bean="personInstance" property="minor"/>', [personInstance: personInstance])
                == applyTemplate('<g:formatBoolean boolean="${personInstance.minor}"/>', [personInstance: personInstance])
    }

    void 'f:displayWidget without template and a Boolean value renders the formatted Boolean'() {
        expect:
        applyTemplate('<f:displayWidget bean="personInstance" property="grailsDeveloper"/>', [personInstance: personInstance])
                == applyTemplate('<g:formatBoolean boolean="${personInstance.grailsDeveloper}"/>', [personInstance: personInstance])
    }

    void 'f:displayWidget without template and a regular value renders the field value'() {
        expect:
        applyTemplate('<f:displayWidget bean="personInstance" property="salutation"/>', [personInstance: personInstance])
                == applyTemplate('<g:fieldValue bean="${personInstance}" field="salutation"/>', [personInstance: personInstance])
    }
