...
class DefaultInputRenderingPersistentSpec extends AbstractFormFieldsTagLibSpec implements TagLibUnitTest<FormFieldsTagLib> {
    ...
    @Shared
    def personDomainClass = Mock(PersistentEntity) {
        getJavaClass() >> Person
        getDecapitalizedName() >> 'person'
    }

    @Shared
    def basicProperty = Mock(Basic)

    @Shared
    def oneToOneProperty = Mock(OneToOne) {
        getAssociatedEntity() >> personDomainClass
    }

    @Shared
    def manyToOneProperty = Mock(ManyToOne) {
        getAssociatedEntity() >> personDomainClass
    }

    @Shared
    def manyToManyProperty = Mock(ManyToMany) {
        getAssociatedEntity() >> personDomainClass
    }

    @Shared
    def oneToManyProperty = Mock(OneToMany) {
        getAssociatedEntity() >> personDomainClass
    }
    ...
    def 'a one-to-many property has a list of links instead of an input'() {
        ...
        and:
        def model = [bean: [id: 1337], beanClass: personDomainClass, type: Set,
                     property: 'prop', constraints: null, persistentProperty: oneToManyProperty, value: people]
    ...
    def 'an enum with a toString method uses name instead of toString for the keys'() {
        ...
        EnumWithToString.values().every {
            output =~ /option value="${it.name()}" >${it}</
        }
    }
    ...
    def 'enum select should respect constraint inList'() {
        given:
        def model = [type: EnumWithToString, property: 'prop',
                     constraints: buildConstraints([inList: inListConstraint]), persistentProperty: basicProperty]
