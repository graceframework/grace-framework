...
class DefaultInputRenderingSpec extends AbstractFormFieldsTagLibSpec implements TagLibUnitTest<FormFieldsTagLib> {
    ...
    @Shared
    def basicProperty = Mock(Association)

    @Shared
    def oneToOneProperty

    @Shared
    def manyToOneProperty

    @Shared
    def manyToManyProperty
    ...
    def 'a one-to-many property has a list of links instead of an input'() {
        ...
        and:
        def model = [bean: [id: 1337], beanClass: Mock(PersistentEntity) { getDecapitalizedName() >> 'thing' },
                     type: Set, property: 'prop', constraints: null, persistentProperty: oneToManyProperty, value: people]
