...
|*template*|String|'table'|Alternative template for table rendering. Templates should be in the 'app/views/templates/_fields/' folder.
|===
...
To make you own version of a `f:table` template, the file should be located in `app/views/templates/_fields/_table.gsp`
unless a different location is specified with the template property.
You can find a starting point for a new file link:https://github.com/graceframework/grace-fields/blob/6.3.x/app/views/templates/_fields/_table.gsp[on GitHub]
