...
|*property*
| No
|The path to the property. This can include any level of nesting and numeric or string indexes. For example `employees[0].address[home].street` is a valid path. If absent, all properties will be displayed via the `app/views/templates/_fields/_list.gsp` (see the template code below).
...
When `f:display` is used without a `property` then all bean properties are rendered. This is done with the `app/views/templates/_fields/_list.gsp` template. The default template can be found link:https://github.com/graceframework/grace-fields/blob/6.3.x/app/views/templates/_fields/_list.gsp[on GitHub]