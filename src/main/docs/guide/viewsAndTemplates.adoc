...
The link:{controllersRef}/render.html[render] controller method writes directly to the response, which is the most common behaviour. To instead obtain the result of template as a String you can use the link:../ref/Tags/render.html[render] tag:
...
Notice the usage of the `g` namespace which tells Grails we want to use the <<tagsAsMethodCalls,tag as method call>> instead of the link:{controllersRef}/render.html[render] method.
