...
class StackTracePrinterSpec extends Specification {
    ...
    void "Test pretty print simple stack trace"() {
        given: "a controller that throws an exception"
            ...
            gcl.parseClass(new InputStreamReader(getServiceResource().inputStream), serviceResource.filename)
            def controller = gcl.parseClass(new InputStreamReader(getControllerResource().inputStream), controllerResource.filename).newInstance()
    ...
    void "Test pretty print code snippet"() {
        given: "a controller that throws an exception"
            final gcl = new GroovyClassLoader()
            gcl.parseClass(new InputStreamReader(getServiceResource().inputStream), serviceResource.filename)
            def controller = gcl.parseClass(new InputStreamReader(getControllerResource().inputStream), getControllerResource().filename).newInstance()
...
'''
    ...
    void "Test pretty print nested exception code snippet"() {
        given:"a service that throws an exception that is caught and rethrown"
            final gcl = new GroovyClassLoader()
            gcl.parseClass(new InputStreamReader(getServiceResource().inputStream), serviceResource.filename)
            def controller = gcl.parseClass(new InputStreamReader(controllerResource.inputStream), controllerResource.filename).newInstance()
            final locator = new StaticResourceLocator()
