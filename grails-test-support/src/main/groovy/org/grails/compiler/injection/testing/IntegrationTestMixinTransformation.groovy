...
class IntegrationTestMixinTransformation implements ASTTransformation {
    ...
    void weaveIntegrationTestMixin(ClassNode classNode, ClassNode applicationClassNode) {
        ...
        // now add integration test annotations @SpringBootTest
        def servletApi = null
        try {
        ...
        if (servletApi != null) {
            if (GrailsASTUtils.findAnnotation(classNode, SpringBootTest) == null) {
                ...
                webIntegrationTestAnnotation.addMember('webEnvironment',
                        propX(classX(SpringBootTest.WebEnvironment), 'RANDOM_PORT'))

                if (classNode.getProperty('serverPort') == null) {
                    def serverPortField = new FieldNode('serverPort', Modifier.PROTECTED, ClassHelper.Integer_TYPE,
                            classNode, new ConstantExpression(8080))
    ...
    protected void enableAutowireByName(ClassNode classNode) {
        ...
        def getBeanFactoryMethodCall = new MethodCallExpression(
                new VariableExpression(p), 'getAutowireCapableBeanFactory', GrailsASTUtils.ZERO_ARGUMENTS)

        def args = new ArgumentListExpression(new VariableExpression('this'),
                new ConstantExpression(AutowireCapableBeanFactory.AUTOWIRE_BY_NAME), new ConstantExpression(Boolean.FALSE))
    ...
    protected void enhanceGebSpecWithPort(ClassNode classNode) {
        if (GrailsASTUtils.isSubclassOf(classNode, 'geb.spock.GebSpec')) {
            ...
            Expression urlExpression = new GStringExpression('http://localhost:${serverPort}${serverContextPath}',
                    [new ConstantExpression('http://localhost:'), new ConstantExpression(''), new ConstantExpression('')],
                    [new VariableExpression('serverPort'), new VariableExpression('serverContextPath')] as List<Expression>)

            Expression baseUrlVariableExpression = new VariableExpression('$baseUrl', ClassHelper.make(String))
            Expression declareBaseUrlExpression = new DeclarationExpression(baseUrlVariableExpression,
                    Token.newSymbol(Types.EQUALS, 0, 0),
                    new MethodCallExpression(urlExpression, 'toString', new ArgumentListExpression()))

            Expression constantSlashExpression = new ConstantExpression('/')
            Expression endsWithMethodCall = new MethodCallExpression(baseUrlVariableExpression, 'endsWith',
                    new ArgumentListExpression(constantSlashExpression))
            Expression appendSlashExpression = new BinaryExpression(baseUrlVariableExpression,
                    Token.newSymbol(Types.PLUS_EQUAL, 0, 0), constantSlashExpression)
            Statement ifUrlEndsWithSlashStatement = new IfStatement(new BooleanExpression(endsWithMethodCall),
                    new EmptyStatement(), new ExpressionStatement(appendSlashExpression))
            ...
            def method = new MethodNode('configureGebBaseUrl',
                    Modifier.PUBLIC, ClassHelper.VOID_TYPE,
                    [contextPathParameter, serverPortParameter] as Parameter[], null, methodBody)
